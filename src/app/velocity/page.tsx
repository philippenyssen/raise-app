...
const tierBadgeBase: React.CSSProperties = { width: '20px', height: '20px', fontSize: 'var(--font-size-xs)', flexShrink: 0 };
const tierBadgeHigh: React.CSSProperties = { ...tierBadgeBase, background: 'var(--accent)', color: 'var(--text-primary)' };
const tierBadgeLow: React.CSSProperties = { ...tierBadgeBase, background: 'var(--surface-3)', color: 'var(--text-secondary)' };
              ...
              <tbody>
                {investors.map((inv) => (
                  <tr
                    ...
                    <td style={cellCenter}>
                      <span
                        style={{ ...tabNumBase, color: daysProcessColor(inv.days_in_process) }}>
                        {inv.days_in_process}d</span></td>

                    {/* Days in Stage */}
                    <td style={cellCenter}>
                      <span
                        style={{ ...tabNumBase, color: daysStageColor(inv.days_in_current_stage) }}>
