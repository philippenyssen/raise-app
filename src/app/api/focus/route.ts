...
interface FocusItem {
  investorId: string; investorName: string; investorType: string; investorTier: number;
  status: string; enthusiasm: number; focusScore: number;
...
function computeUrgencyScore(daysSinceLastMeeting: number | null, tier: number, status: string): number {
  let recencyScore = 0;
  if (daysSinceLastMeeting === null) {
    const statusUrgency: Record<string, number> = { identified: 20, contacted: 40, nda_signed: 60, meeting_scheduled: 70, met: 50, engaged: 80, in_dd: 90, term_sheet: 95 };
    recencyScore = statusUrgency[status] ?? 30;
  } else if (daysSinceLastMeeting <= 3) recencyScore = 30;
  else if (daysSinceLastMeeting <= 7) recencyScore = 50;
  else if (daysSinceLastMeeting <= 14) recencyScore = 70;
  else if (daysSinceLastMeeting <= 21) recencyScore = 85;
  else if (daysSinceLastMeeting <= 30) recencyScore = 95;
  else recencyScore = 100;

  const tierMultiplier: Record<number, number> = { 1: 1.0, 2: 0.85, 3: 0.7, 4: 0.55 };
  const statusMultiplier: Record<string, number> = { identified: 0.5, contacted: 0.6, nda_signed: 0.7, meeting_scheduled: 0.8, met: 0.75, engaged: 0.9, in_dd: 1.0, term_sheet: 1.0 };
  return clamp(recencyScore * (tierMultiplier[tier] ?? 0.7) * (statusMultiplier[status] ?? 0.6));
}

function computeMomentumRiskScore(meetings: Meeting[], investor: Investor): number {
  if (meetings.length === 0) {
    const statusRisk: Record<string, number> = { identified: 20, contacted: 40, nda_signed: 50, meeting_scheduled: 30, engaged: 70, in_dd: 80 };
  ...
  if (sorted.length >= 2) {
    ...
    if (latestEnth < prevEnth) riskScore += (prevEnth - latestEnth) * 20;
    if (latestEnth <= 2) riskScore += 25;
  }
  ...
  riskScore += allObjections.filter(o => o.severity === 'showstopper' && o.response_effectiveness !== 'resolved').length * 15;

  const latestSignals = parseJsonSafe<Record<string, unknown>>(latest.engagement_signals, {});
  if (!latestSignals.requested_followup && meetings.length >= 2) riskScore += 10;
...
function computeOpportunitySizeScore(investor: Investor): number {
  ...
  const typeBonus: Record<string, number> = { sovereign: 15, growth: 10, vc: 0, strategic: 5, debt: -10, family_office: -5 };
  score += typeBonus[investor.type] ?? 0;
  if (investor.speed === 'fast') score += 5;
  return clamp(score);
}

function computeActionReadinessScore(pendingTaskCount: number, openFlagCount: number, unresolvedObjections: number, status: string): number {
  let score = Math.min(40, pendingTaskCount * 15) + Math.min(30, openFlagCount * 10) + Math.min(30, unresolvedObjections * 10);
  const statusBonus: Record<string, number> = { nda_signed: 15, meeting_scheduled: 10, engaged: 10, in_dd: 20, term_sheet: 25 };
...
function determineRecommendedAction(
  investor: Investor, meetings: Meeting[], daysSince: number | null,
  unresolvedObjections: string[], pendingTaskCount: number, momentum: string, topObjectionTopic: string | null,
): { action: string; timeEstimate: string; expectedImpact: string; riskIfIgnored: string } {
  const sorted = meetings.length > 0 ? [...meetings].sort((a, b) => b.date.localeCompare(a.date)) : [];
  const latest = sorted[0] ?? null;
  const latestEnth = latest?.enthusiasm_score ?? investor.enthusiasm;
  const partner = investor.partner || investor.name;

  if (investor.status === 'term_sheet') return { action: `Negotiate terms with ${partner} --- all signals are positive, don't let momentum fade`, timeEstimate: '2hr meeting + 1hr prep', expectedImpact: `If closed this week: secures anchor commitment at target valuation`, riskIfIgnored: `Term sheet may expire or investor redirects capital to competing deal` };

  if (investor.status === 'in_dd') {
    if (pendingTaskCount > 0) return { action: `Respond to outstanding DD requests from ${partner} --- ${pendingTaskCount} pending items need CEO input`, timeEstimate: '1hr prep + 30min review', expectedImpact: `Fast DD response signals operational excellence; conviction likely rises to ${Math.min(5, latestEnth + 1)}/5`, riskIfIgnored: `${Math.max(1, Math.round(daysSince ?? 7))} days without DD response signals disorganization` };
    return { action: `Push for term sheet with ${partner} --- DD is progressing, propose timeline for IC decision`, timeEstimate: '30min call', expectedImpact: `If term sheet received: anchors the round and creates FOMO for other investors`, riskIfIgnored: `DD fatigue sets in after 3+ weeks without clear next step` };
  }

  if (unresolvedObjections.length > 0) return { action: `Address "${unresolvedObjections[0]}" raised by ${partner} --- prepare data-backed response before enthusiasm drops further`, timeEstimate: '45min prep + 15min call', expectedImpact: `Resolving top objection typically moves enthusiasm from ${latestEnth}/5 to ${Math.min(5, latestEnth + 1)}/5`, riskIfIgnored: `Unresolved objection becomes internal narrative at IC --- harder to reverse once embedded` };

  if (momentum === 'stalled' && meetings.length > 0) return { action: `Re-engage ${partner} with a value-add update --- share recent milestone or market data to restart conversation`, timeEstimate: '30min prep + 15min call', expectedImpact: `Re-engagement within 48 hours recovers 70%+ of stalled processes`, riskIfIgnored: `${Math.round(daysSince ?? 14)}+ days of silence --- investor likely reallocates attention` };

  if (momentum === 'decelerating') {
    if (topObjectionTopic) return { action: `Send updated materials addressing ${topObjectionTopic} concerns to ${partner} before next meeting`, timeEstimate: '30min prep', expectedImpact: `Proactive objection handling signals conviction and keeps momentum alive`, riskIfIgnored: `Decelerating processes rarely re-accelerate without proactive intervention` };
    return { action: `Schedule a focused call with ${partner} --- propose specific agenda to maintain cadence`, timeEstimate: '15min prep + 30min call', expectedImpact: `Consistent cadence prevents the "we'll circle back" fade`, riskIfIgnored: `3 more days of deceleration shifts investor attention to other opportunities` };
  }

  switch (investor.status) {
    case 'identified': return { action: `Activate warm intro through ${investor.warm_path || 'best available path'} to reach ${partner}`, timeEstimate: '15min outreach', expectedImpact: `Warm intros convert at 3-5x the rate of cold outreach`, riskIfIgnored: `Competitor may reach them first with a conflicting deal` };
    case 'contacted': return { action: `Follow up with ${partner} --- send tailored one-pager and propose 30-min intro call`, timeEstimate: '15min prep + email', expectedImpact: `Second touch within 5 days doubles response rate`, riskIfIgnored: `Initial outreach fades from inbox after 7-10 days` };
    case 'nda_signed': return { action: `Schedule deep dive with ${partner} --- NDA is signed, they're ready for the full management presentation`, timeEstimate: '30min prep + 1hr meeting', expectedImpact: `Moving to management presentation accelerates timeline by 2-3 weeks`, riskIfIgnored: `NDA without follow-up signals lack of preparation` };
    case 'meeting_scheduled': return { action: `Prepare for upcoming meeting with ${partner} --- review their portfolio for discussion points and prepare for likely objections`, timeEstimate: '45min prep', expectedImpact: `Tailored preparation dramatically improves first-meeting conversion`, riskIfIgnored: `Generic presentation risks losing a high-potential investor at first contact` };
    case 'met': {
      if (latest?.next_steps) return { action: `Execute next steps from ${partner} meeting: ${latest.next_steps.substring(0, 100)}`, timeEstimate: '30min execution', expectedImpact: `Completing next steps within 48 hours shows execution quality`, riskIfIgnored: `Delayed follow-up is the #1 reason investors lose interest post-meeting` };
      return { action: `Send follow-up materials to ${partner} and propose deep dive on financial model`, timeEstimate: '30min prep', expectedImpact: `Fast follow-up within 48 hours converts 60%+ of positive first meetings`, riskIfIgnored: `Every day of delay reduces conversion probability by ~5%` };
    }
    case 'engaged': {
      if (latestEnth >= 4) return { action: `Push ${partner} toward DD --- enthusiasm is high (${latestEnth}/5), propose structured DD timeline`, timeEstimate: '30min call', expectedImpact: `Entering DD at peak enthusiasm maximizes conversion probability`, riskIfIgnored: `High enthusiasm is perishable --- peaks don't last more than 2-3 weeks` };
...
export async function GET() {
  try {
    const db = getClient();
    const [investorRows, allMeetings, taskRows, flagRows, raiseConfig, allPortfolios] = await Promise.all([
    ...
    (taskRows.rows as unknown as Array<{ investor_id: string; count: number }>).forEach(r => { taskCountByInvestor[r.investor_id] = Number(r.count); });
    const flagCountByInvestor: Record<string, number> = {};
    (flagRows.rows as unknown as Array<{ investor_id: string; count: number }>).forEach(r => { flagCountByInvestor[r.investor_id] = Number(r.count); });
    ...
    for (const investor of investors) {
      ...
      const daysSinceLastMeeting = latestMeeting ? Math.round(daysBetween(latestMeeting.date, now)) : null;
      ...
      const allObjections = meetings.flatMap(m => parseJsonSafe<Objection[]>(m.objections, []));
      const unresolvedObjections = allObjections.filter(o => o.response_effectiveness !== 'resolved' && (o.severity === 'showstopper' || o.severity === 'significant')).map(o => o.text);
      const topObjectionTopic = allObjections.filter(o => o.response_effectiveness !== 'resolved').map(o => o.topic).filter(Boolean)[0] ?? null;

      const investorScoreComponent = investorScore.overall;
    ...
    const staleAlerts = focusItems.filter(item => engagedStatuses.includes(item.status) && (item.daysSinceLastMeeting === null || item.daysSinceLastMeeting >= 14))
      .sort((a, b) => (b.daysSinceLastMeeting ?? 999) - (a.daysSinceLastMeeting ?? 999));

    const timeEstimates: Record<string, number> = {
      '15min outreach': 0.25, '15min prep + email': 0.25, '15min prep + 30min call': 0.75, '15min call': 0.25, '15min review': 0.25,
      '30min prep': 0.5, '30min call': 0.5, '30min prep + 15min call': 0.75, '30min prep + 1hr meeting': 1.5, '30min execution': 0.5,
      '45min prep': 0.75, '45min prep + 15min call': 1, '1hr meeting': 1, '1hr prep + 30min review': 1.5, '2hr meeting + 1hr prep': 3,
    };

    let totalHours = 0, meetingsCount = 0, followUpsCount = 0;
    const topItems = focusItems.slice(0, 15);
    for (const item of topItems) {
      totalHours += timeEstimates[item.timeEstimate] ?? 0.5;
      if (item.timeEstimate.includes('meeting') || item.timeEstimate.includes('call')) meetingsCount++;
      else followUpsCount++;
    }
    ...
    return NextResponse.json({ error: 'Couldn\'t analyze your investor data — check your connection and refresh' }, { status: 500 });
  }}
