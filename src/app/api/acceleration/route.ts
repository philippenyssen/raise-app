...
interface AccelerationItem {
  ...
  score: number;
  momentum: string;
  triggerType: 'momentum_cliff' | 'stall_risk' | 'window_closing' | 'catalyst_match' | 'competitive_pressure' | 'term_sheet_ready';
  actionType: 'milestone_share' | 'expert_call' | 'site_visit' | 'competitive_signal' | 'warm_reintro' | 'data_update' | 'escalation';
  description: string;
  expectedLift: number;
  confidence: 'high' | 'medium' | 'low';
  timeEstimate: '15min' | '30min' | '1hr' | '2hr' | 'half_day';
  urgency: 'immediate' | '48h' | 'this_week' | 'next_week';
...
function detectMomentumCliff(
  investor: Investor,
  meetings: Meeting[],
): AccelerationItem | null {
  if (meetings.length < 2) return null;

  const sorted = [...meetings].sort((a, b) => a.date.localeCompare(b.date));
  const recent = sorted.slice(-3); // Last 3 meetings

  if (recent.length < 2) return null;

  // Check for enthusiasm decline
  ...
  // Check for unresolved objections driving the decline
  const latestMeeting = sorted[sorted.length - 1];
  const objections = parseJsonSafe<Objection[]>(latestMeeting.objections, []);
  const unresolvedShowstoppers = objections.filter(
    o => o.response_effectiveness !== 'resolved' && (o.severity === 'showstopper' || o.severity === 'significant')
  );

  let description: string;
  let actionType: AccelerationItem['actionType'];
  let timeEstimate: AccelerationItem['timeEstimate'];

  if (unresolvedShowstoppers.length > 0) {
    const topObj = unresolvedShowstoppers[0];
    description = `Enthusiasm dropping (${scores.join(' -> ')}/5). Unresolved objection on "${topObj.text}" is likely driving the decline. Send targeted response with updated data within 48 hours.`;
  ...
  return {
    ...
    actionType,
    description,
    expectedLift: drop >= 1.5 ? 10 : 15,
    confidence: drop >= 1.5 ? 'medium' : 'high',
    timeEstimate,
    urgency: 'immediate',
    triggerEvidence: `Enthusiasm trend: ${scores.join(' -> ')}/5 over last ${scores.length} meetings`,
  };
}

function detectStallRisk(
  investor: Investor,
  meetings: Meeting[],
  now: string,
): AccelerationItem | null {
  const statusIdx = STATUS_PROGRESSION[investor.status] ?? 0;
  if (statusIdx <= 0 || statusIdx >= 8) return null; // skip passed/dropped/closed
  ...
  // Decide action based on score context
  ...
  let actionType: AccelerationItem['actionType'];
  let timeEstimate: AccelerationItem['timeEstimate'];
  let urgency: AccelerationItem['urgency'];
  let expectedLift: number;
  let confidence: AccelerationItem['confidence'];

  if (isVeryStale && !isHighConviction) {
    // Low score + very stale = deprioritize
    ...
    // High score + going cold = re-engage
  ...
  return {
    ...
    timeEstimate,
    urgency,
    triggerEvidence: isStaleStatus
      ? `Status "${investor.status}" unchanged for ${Math.round(daysSinceUpdate)}d, enthusiasm ${latestEnth}/5`
      : `${Math.round(daysSinceLastMeeting!)}d since last meeting`,
  };
}
...
function detectWindowClosing(
  ...
  if (daysToClose > 120) return null; // not urgent yet

  const statusIdx = STATUS_PROGRESSION[investor.status] ?? 0;
  // Only flag if investor is in early stages
  if (statusIdx >= 5) return null; // engaged+ is probably fine
  if (statusIdx <= 0) return null; // passed/dropped/identified

  // Estimate how long it takes to get from current status to term sheet
  // Typical: contacted -> NDA (5d) -> meeting (7d) -> engaged (14d) -> DD (21d) -> term sheet (14d) = ~61d
  const stepsRemaining = 7 - statusIdx;
  const estimatedDaysNeeded = stepsRemaining * 14; // rough 2 weeks per stage

  if (estimatedDaysNeeded <= daysToClose) return null; // they can make it

  const partner = investor.partner || investor.name;
  const description = `${partner} is in "${investor.status}" with ~${Math.round(daysToClose)}d until target close. At current pace, they need ~${estimatedDaysNeeded}d to reach term sheet. Accelerate by sharing complete data room access and offering reference calls to compress the DD timeline.`;

  return {
    id: `wc_${investor.id}_${Date.now()}`,
    investorId: investor.id,
    investorName: investor.name,
    investorTier: investor.tier,
    investorType: investor.type,
    ...
    score: 0,
    momentum: 'steady',
    triggerType: 'window_closing',
    actionType: 'data_update',
    description,
    expectedLift: 10,
    confidence: 'medium',
    timeEstimate: '1hr',
    urgency: daysToClose < 45 ? 'immediate' : '48h',
    triggerEvidence: `${Math.round(daysToClose)}d to target close, ~${estimatedDaysNeeded}d needed at current pace`,
  };
}

function detectCompetitivePressure(
  investor: Investor,
  meetings: Meeting[],
  ...
  // Find investors in same tier that recently moved forward
  const recentAccelerators = allInvestors.filter(other => {
    if (other.id === investor.id) return false;
    if (other.tier !== investor.tier) return false;
    const otherIdx = STATUS_PROGRESSION[other.status] ?? 0;
    if (otherIdx <= statusIdx) return false;
    // Check if updated recently (within 7 days)
    const daysSinceUpdate = daysBetween(other.updated_at, now);
  ...
  const description = `${movingNames} recently advanced to a later stage. Mention process momentum to ${partner} without naming specifics --- competitive dynamics create urgency that generic follow-ups cannot.`;

  return {
    id: `cp_${investor.id}_${Date.now()}`,
    investorId: investor.id,
    investorName: investor.name,
    investorTier: investor.tier,
    investorType: investor.type,
    ...
    actionType: 'competitive_signal',
    description,
    expectedLift: 10,
    confidence: 'medium',
    timeEstimate: '15min',
    urgency: '48h',
    triggerEvidence: `${recentAccelerators.length} peer investor(s) in Tier ${investor.tier} moved forward in last 7d`,
  };
}

function detectTermSheetReadiness(
  investor: Investor,
  meetings: Meeting[],
  score: number,
  momentum: string,
): AccelerationItem | null {
  if (score < 70) return null; // need high score
  if (investor.status === 'term_sheet' || investor.status === 'closed') return null;
  if (investor.status === 'passed' || investor.status === 'dropped') return null;

  // Check if momentum is accelerating or at least steady
  if (momentum === 'stalled' || momentum === 'decelerating') return null;

  // Check if major objections are resolved
  ...
  if (unresolvedShowstoppers.length > 0) return null;

  // Check enthusiasm is strong
  const sorted = [...meetings].sort((a, b) => b.date.localeCompare(a.date));
  const latestEnth = sorted[0]?.enthusiasm_score ?? investor.enthusiasm;
  if (latestEnth < 4) return null;

  const partner = investor.partner || investor.name;
  const description = `${partner} shows all green signals: score ${score}/100, momentum ${momentum}, enthusiasm ${latestEnth}/5, no unresolved showstoppers. Push for term sheet within 10 days --- propose a specific timeline for IC submission.`;

  return {
    id: `ts_${investor.id}_${Date.now()}`,
    investorId: investor.id,
    investorName: investor.name,
    investorTier: investor.tier,
    investorType: investor.type,
    ...
    timeEstimate: '30min',
    urgency: 'immediate',
    triggerEvidence: `Score ${score}/100, momentum ${momentum}, enthusiasm ${latestEnth}/5, 0 showstoppers`,
  };
}

// ---------------------------------------------------------------------------
// GET Handler — Run acceleration analysis for all active investors
// ---------------------------------------------------------------------------
...
export async function GET() {
  try {
    ...
    for (const investor of investors) {
      ...
      const triggers: (AccelerationItem | null)[] = [
        detectMomentumCliff(investor, meetings),
        detectStallRisk(investor, meetings, now),
        detectWindowClosing(investor, meetings, targetCloseDate, now),
        detectCompetitivePressure(investor, meetings, investors, now),
        detectTermSheetReadiness(investor, meetings, score, momentum),
      ];
      ...
      for (const trigger of triggers) {
        if (trigger) {
          ...
          // Categorize
          if (trigger.triggerType === 'term_sheet_ready') {
            termSheetReady.push({
              investorId: investor.id,
              investorName: investor.name,
              investorTier: investor.tier,
              ...
              score,
              momentum,
              reason: trigger.triggerEvidence,
            });
          } else if (
            trigger.triggerType === 'stall_risk' &&
            trigger.urgency === 'next_week' &&
            ...
            trigger.triggerType === 'momentum_cliff' ||
            (trigger.triggerType === 'stall_risk' && trigger.urgency !== 'next_week')
          ) {
            // Only add to atRisk if not already there
            if (!atRisk.find(r => r.investorId === investor.id)) {
              atRisk.push({
                investorId: investor.id,
                investorName: investor.name,
                investorTier: investor.tier,
    ...
    // Sort accelerations by urgency priority
    const urgencyOrder: Record<string, number> = {
      immediate: 0, '48h': 1, this_week: 2, next_week: 3,
    };
    ...
    const immediateCount = accelerations.filter(a => a.urgency === 'immediate').length;
    const thisWeekCount = accelerations.filter(a => a.urgency === '48h' || a.urgency === 'this_week').length;

    return NextResponse.json({
      summary: {
        immediate: immediateCount,
        this_week: thisWeekCount,
        total: accelerations.length,
      },
      accelerations,
      termSheetReady,
      atRisk,
    ...
    return NextResponse.json(
      { error: 'Failed to run acceleration analysis', detail: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 },
    );
  }
}
...
// ---------------------------------------------------------------------------
// PUT Handler — Update an acceleration action (mark executed, record lift)
// ---------------------------------------------------------------------------
...
export async function PUT(req: Request) {
  try {
    ...
    return NextResponse.json(
      { error: 'Failed to update acceleration action', detail: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 },
    );
  }
}
