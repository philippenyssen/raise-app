...
const STAGE_WEIGHTS: Record<string, number> = { identified: 0, contacted: 1, nda_signed: 2, meeting_scheduled: 3, met: 4, engaged: 5, in_dd: 6, term_sheet: 8, closed: 10, passed: 0, dropped: 0 };
...
export async function GET() {
  try {
    ...
    const activities = activityRows.rows as unknown as Array<{ id: string; event_type: string; subject: string; detail: string; investor_id: string; investor_name: string; created_at: string }>;

    const statusCounts: Record<string, number> = {};
    (statusCountRows.rows as unknown as Array<{ status: string; count: number }>).forEach(r => { statusCounts[r.status] = Number(r.count); });

    let targetCloseDate: string | null = null;
    if (configRow.rows.length > 0) {
      try { targetCloseDate = JSON.parse(configRow.rows[0].value as string).target_close || null; } catch { /* ignore */ }
    }
    ...
    for (let i = 0; i < PIPELINE_ORDER.length - 1; i++) {
      const from = PIPELINE_ORDER[i], to = PIPELINE_ORDER[i + 1];
      const fromCount = funnelCumulative[from] || 0, toCount = funnelCumulative[to] || 0;
      conversionRates.push({ from, to, rate: fromCount > 0 ? Math.round((toCount / fromCount) * 100) : 0, fromCount, toCount });
    }
    ...
    for (let i = 0; i < PIPELINE_ORDER.length - 1; i++) {
      const from = PIPELINE_ORDER[i], to = PIPELINE_ORDER[i + 1];
      const fromCount = funnelCumulative[from] || 0, toCount = funnelCumulative[to] || 0;
      dropOffRates.push({ stage: from, rate: fromCount > 0 ? Math.round(((fromCount - toCount) / fromCount) * 100) : 0, count: fromCount - toCount });
    }
    ...
    activities.forEach(a => {
      if (a.event_type === 'status_changed' && a.investor_id) {
        ...
        if (!newStatus && subjectMatch) newStatus = subjectMatch[1].toLowerCase();
    ...
    const funnel = {
      ...
      bottleneck: bottleneckStage ? { stage: bottleneckStage, label: STAGE_LABELS[bottleneckStage] || bottleneckStage, count: bottleneckCount } : null,
    };
    ...
    for (let w = 7; w >= 0; w--) {
      ...
      const count = meetings.filter(m => { const d = new Date(m.date); return d >= weekStart && d < weekEnd; }).length;
    ...
    for (let w = 7; w >= 0; w--) {
      const weekStart = new Date(now); weekStart.setDate(weekStart.getDate() - (w * 7));
      const weekEnd = new Date(weekStart); weekEnd.setDate(weekEnd.getDate() + 7);
      const count = investors.filter(inv => { const d = new Date(inv.created_at); return d >= weekStart && d < weekEnd; }).length;
      const weekLabel = `W${Math.ceil((weekStart.getDate()) / 7)} ${weekStart.toLocaleString('default', { month: 'short' })}`;
    ...
    const velocityScore = activeInvestors.length > 0
      ? Math.round((activeInvestors.reduce((sum, inv) => sum + (TIER_WEIGHTS[inv.tier] || 1) * (STAGE_WEIGHTS[inv.status] || 0) * Math.max(inv.enthusiasm || 1, 1), 0) / activeInvestors.length) * 10) / 10
      : 0;

    const lastMeetingDate = meetings.length > 0 ? new Date(meetings[0].date) : null;
    const lastActivityDate = activities.length > 0 ? new Date(activities[activities.length - 1].created_at) : null;
    const lastProgressDate = [lastMeetingDate, lastActivityDate].filter(Boolean).sort((a, b) => (b?.getTime() || 0) - (a?.getTime() || 0))[0];
    const daysSinceProgress = lastProgressDate ? Math.round((now.getTime() - lastProgressDate.getTime()) / (1000 * 60 * 60 * 24)) : null;

    let estimatedDaysToClose: number | null = null;
    let onTrack: boolean | null = null;
    if (targetCloseDate) {
      const daysRemaining = Math.round((new Date(targetCloseDate).getTime() - now.getTime()) / (1000 * 60 * 60 * 24));
    ...
    let totalObjections = 0, addressedObjections = 0;

    meetings.forEach(m => {
      try {
        const objs = JSON.parse(m.objections || '[]') as Array<{ text: string; topic: string; severity?: string; addressed?: boolean; response_effectiveness?: string }>;
        objs.forEach(o => {
          totalObjections++;
    ...
    const engagement = {
      enthusiasmByType,
      avgEnthusiasm: investors.filter(i => i.enthusiasm > 0).length > 0 ? Math.round((investors.filter(i => i.enthusiasm > 0).reduce((s, i) => s + i.enthusiasm, 0) / investors.filter(i => i.enthusiasm > 0).length) * 10) / 10 : 0,
      topObjections, objectionResolutionRate, totalObjections, addressedObjections, competitiveMentions,
      topCompetitors: Object.entries(competitorNames).map(([name, count]) => ({ name, count })).sort((a, b) => b.count - a.count).slice(0, 5),
    };
    ...
    const staleInvestors: Array<{ id: string; name: string; status: string; tier: number; type: string; lastMeetingDate: string | null; daysSinceLastMeeting: number | null }> = [];

    const meetingsByInvestor: Record<string, string> = {};
    meetings.forEach(m => {
      if (!meetingsByInvestor[m.investor_id] || m.date > meetingsByInvestor[m.investor_id]) meetingsByInvestor[m.investor_id] = m.date;
    });
    ...
    Object.entries(meetingsByInvestorAll).forEach(([invId, mtgs]) => {
      if (mtgs.length >= 2) {
        ...
        const latest = mtgs[mtgs.length - 1], previous = mtgs[mtgs.length - 2];
    ...
    const maxTypeConcentration = activeInvestors.length > 0 ? Math.max(...Object.values(investorsByType)) / activeInvestors.length : 0;
    const concentrationRisk = maxTypeConcentration > 0.6;
    const dominantType = Object.entries(investorsByType).sort((a, b) => b[1] - a[1])[0];
    ...
    const risks = {
      ...
      concentrationRisk: { isRisky: concentrationRisk, maxConcentration: Math.round(maxTypeConcentration * 100), dominantType: dominantType ? dominantType[0] : null, breakdown: investorsByType },
      timelineRisk: { level: timelineRisk, daysRemaining: estimatedDaysToClose, targetDate: targetCloseDate },
      totalAlerts: staleInvestors.length + decliningEnthusiasm.length + highTierStuck.length + (concentrationRisk ? 1 : 0) + (timelineRisk !== 'low' ? 1 : 0),
    };
