...
| 45 | 2026-03-14 | Core UX Reliability | **Loading skeletons** for 7 core pages (Today, Dashboard, Pipeline, Investors, Meetings, Follow-ups, Focus). **Error boundaries** with retry + dashboard fallback for same 7 pages. **Today page polish**: staleness indicator (warning at 5min), auto-refresh bar, empty states for meetings/actions. **JSON.parse safety**: 7 bare parse calls wrapped in try/catch across 6 files (db.ts, generate.ts, enrichment, settings, meetings/new, documents/adapt, meeting-brief). | today/loading.tsx, loading.tsx, pipeline/loading.tsx, investors/loading.tsx, meetings/loading.tsx, followups/loading.tsx, focus/loading.tsx, today/error.tsx, error.tsx, pipeline/error.tsx, investors/error.tsx, meetings/error.tsx, followups/error.tsx, focus/error.tsx, today/page.tsx, db.ts, generate.ts, enrichment/page.tsx, api/settings/route.ts, meetings/new/page.tsx, api/documents/adapt/route.ts, api/meeting-brief/route.ts |
| 44 | 2026-03-14 | Navigation Consolidation | **Sidebar**: 35→15 items, organized by workflow (DAILY/EXECUTE/ANALYZE/TOOLS). **Dealflow page**: merged velocity + deal heat + momentum into unified investor health view. **Quick Capture**: buttons added to Today + Meetings pages. **Cross-page links**: 5 broken/stale links fixed. No pages deleted — just hidden from sidebar. | sidebar.tsx, dealflow/page.tsx, meetings/page.tsx, today/page.tsx, page.tsx, api/briefing/route.ts, CLAUDE.md |
| 43 | 2026-03-14 | Data Room + Objections + Dashboard | **3 enhancements to existing pages**: (1) **Data Room Intelligence**: access tracking table, per-investor document access, most requested docs, stage-based recommendations, unreached investor alerts. (2) **Objection Effectiveness**: per-objection success rates, effectiveness leaderboard, worst performers, resolution rates, timeline. (3) **Enriched Dashboard**: pipeline velocity strip, top 5 hot deals by heat score, next 5 due follow-ups with overdue indicators, recent activity feed. Full build passes (44 pages). | data-room/page.tsx, api/data-room/intelligence/route.ts, objections/page.tsx, api/objections/effectiveness/route.ts, page.tsx, db.ts |
| 42 | 2026-03-14 | Win/Loss + Forecast + FOMO | **3 intelligence pages surfacing deep computation**: (1) **Win/Loss Analysis** (`/win-loss`): conversion funnel visualization, winner/loser profiles, pass reasons breakdown, key predictors, time analysis for wins vs losses. (2) **Raise Forecast** (`/forecast`): target progress bar, investor close timeline, confidence distribution, critical path highlight, risk factors, best/base/worst scenarios. (3) **FOMO Dynamics** (`/fomo`): per-investor pressure intensity (0-100), trigger investors, recommended leverage actions, recent pressure events, strategy cards. Sidebar updated. | win-loss/page.tsx, api/win-loss/route.ts, forecast/page.tsx, api/forecast/route.ts, fomo/page.tsx, api/fomo/route.ts, sidebar.tsx |
| 41 | 2026-03-14 | Network Cascade + Deal Heat + Term Sheet Engine | **3 major new pages**: (1) **Network Cascade Visualization** (`/network`): keystone investors with downstream cascade chains, cumulative probability, bottleneck detection, sorted by cascade capital impact. API exposes computeNetworkCascades(). (2) **Deal Heat Map** (`/deal-heat`): composite 0-100 heat score per investor, visual heat grid (hot/warm/cool/cold/frozen), filterable by heat level, top driver displayed per card. (3) **Term Sheet Economics Engine** (`/term-compare`): side-by-side comparison of 2-5 term sheet scenarios, computes post-money, ownership %, effective valuation, founder-friendly score, comparison notes. Terms page links to compare. Sidebar updated with Deal Heat (CORE), Network (INTEL). | network/page.tsx, api/network/route.ts, deal-heat/page.tsx, api/deal-heat/route.ts, term-compare/page.tsx, api/term-compare/route.ts, terms/page.tsx, sidebar.tsx |
...
| 62 | 2026-03-14 | Intelligence Action Buttons | Briefs with investor_id show "Schedule" + "Follow-up" buttons. Research cards show "Schedule Meeting" + "Follow-up" in expanded view. Completes 4-page insight→action audit. | intelligence/page.tsx |
| 61 | 2026-03-14 | Today Overnight Changes | Pulse API overnight data (status changes, new meetings, completed tasks) shown between raise counter and meetings. CEO sees what changed since yesterday first thing. | today/page.tsx |
| 60 | 2026-03-14 | Investor Detail Intelligence Strip | Deal heat score + tracking status + velocity + bottleneck shown below header. At-risk → Rescue, behind → Nudge action buttons. Non-blocking fetch from deal-heat + velocity APIs. Log Meeting passes investor ID. | investors/[id]/page.tsx |
| 59 | 2026-03-14 | Dashboard + Dealflow Action Links | Hot deals: "Schedule" button → meetings/new. Follow-ups: "Act" button → filtered followups. Dealflow bottleneck: smart-linked by keyword (meeting/followup/doc/objection). | page.tsx, dealflow/page.tsx |
| 58 | 2026-03-14 | Strategic Action Routing | Every recommendation gets smart "Take Action" button (keyword-derived: meetings, followups, data-room, objections, forecast, pipeline). Forecast investor names clickable → dealflow search. | strategic/page.tsx |
| 57 | 2026-03-14 | Velocity Action Buttons | Contextual Rescue/Nudge/View buttons per tracking status. At-risk→schedule meeting, behind→send follow-up, on-track→view detail. Closes cross-page navigation gap. | velocity/page.tsx |
| 56 | 2026-03-14 | Today Raise Day Counter | "Day X of 60" progress strip on morning briefing. Progress bar color shifts accent→warning(75%)→danger(over). Days remaining/over label. Fetches velocity API non-blocking. Temporal urgency on the first screen every morning. | today/page.tsx |
| 55 | 2026-03-14 | Forecast What-If Modeling | Interactive investor toggles on forecast timeline. Per-investor estimated capital column. Exclude investors to model scenarios — totals, progress bar, and legend recalculate client-side. Reset button. Zero API changes. | forecast/page.tsx |
| 54 | 2026-03-14 | Focus Page Deal Context | Deal heat flame icon (composite focus+momentum score). Last meeting TYPE shown inline. Open flags count. Top objection topic as "Blocker:" beneath meta row. Reordered meta items for faster scanning. | focus/page.tsx |
| 53 | 2026-03-14 | Pipeline Staleness Indicator | Days-in-stage indicator on each kanban card, color-coded (neutral <7d, warning 7-13d, danger 14d+). Instantly see which deals are stuck without clicking through. | pipeline/page.tsx |
| 52 | 2026-03-14 | Meeting Engagement Intelligence | Per-investor enthusiasm trend arrows (up/down/flat) from meeting history. Meeting count badges for repeat investors. Avg enthusiasm tooltip. All computed client-side from existing data. | meetings/page.tsx |
| 51 | 2026-03-14 | Follow-up Message Drafts | Template-based draft message generation per action type (6 types). PenLine button + expandable draft panel with subject/body + copy-to-clipboard. Stats strip responsive breakpoint (3-col/5-col). Turns follow-ups from tracking into execution. | followups/page.tsx |
| 50 | 2026-03-14 | AI Insight Surfacing | Today page: non-blocking strategic insight card (purple accent, Sparkles icon) from /api/intelligence/strategic. Dashboard: "Day X of 60" pill next to raise progress hero. Both degrade gracefully when data unavailable. | today/page.tsx, page.tsx |
| 49 | 2026-03-14 | Quality of Life | Custom SVG favicon (blue rise arrow), enhanced OG metadata + theme-color meta tag, custom 404 page with Dashboard/Today links. | icon.svg, layout.tsx, not-found.tsx |
| 48 | 2026-03-14 | Loading/Error Coverage + Copy Polish | Loading skeletons + error boundaries for all 8 remaining sidebar pages. All 15 sidebar pages now have consistent loading/error UX. Copy polish on 6 core pages (subtitles, empty states, labels). 22 files. | dealflow/loading+error, forecast/loading+error, objections/loading+error, intelligence/loading+error, documents/loading+error, data-room/loading+error, terms/loading+error, settings/loading+error, page.tsx, pipeline/page.tsx, investors/page.tsx, meetings/page.tsx, followups/page.tsx, focus/page.tsx |
| 47 | 2026-03-14 | Command Palette | Cmd+K command palette: keyboard-first navigation with investor search (fetched on-demand), 15 page shortcuts, 5 quick actions, full arrow/enter navigation, grouped results. Sidebar hint. 445 new lines. | command-palette.tsx, layout.tsx, sidebar.tsx |
| 46 | 2026-03-14 | Dashboard + Pipeline + Follow-ups Polish | Hero raise progress widget (forecast as first element on dashboard), pipeline summary strip (7 metrics), overdue follow-up visual urgency (red borders + sort first). | page.tsx, pipeline/page.tsx, followups/page.tsx |
