...
7. **FIX everything found** — don't just report. Every CRITICAL finding must have a code change in the same cycle. Use parallel agents for independent fixes. A cycle with 3 CRITICAL findings and 0 changes is a FAILED cycle. Default to fixing, not deferring.
8. If a fix is genuinely too complex (>100 lines, requires schema migration, or affects >5 files), implement the minimal safe version now and log the full fix for the next cycle.
9. Verify build passes after fixes
